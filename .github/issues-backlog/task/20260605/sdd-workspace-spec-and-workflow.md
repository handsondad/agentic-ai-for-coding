---
title: 建立 SDD（详细设计驱动开发）文档规范与工作流
type: task
status: published
labels:
- task
...
ai_context: 基于 SDD（详细设计驱动开发）建立统一入口规范，解决文档分散、状态不可追踪、Issue/PR 关联弱的问题。
related_docs:
- SPEC.md
- WORKFLOW.md
- .github/manual/README.md
github_issue_number: '28'
github_issue_url: https://github.com/handsondad/agentic-ai-for-coding/issues/28
---

## 背景与动机

当前详细设计文档缺少统一的目录结构、模板选型规则和状态追踪字段，导致团队协作中出现“文档位置不一致、命名不统一、Issue/PR 难追溯”的问题。
需要建立 SDD 工作区总规范，统一从模板到设计文档再到 Issue/PR 回填的闭环。
...
- 明确 SDD 工作流：
	...
	2) 在 `00-template/` 中选模板。
	3) 在正确领域和日期目录下创建/更新文档。
	4) 创建 Issue 后回填 `issue_status` 与 `issue_url`。
	5) 合并 PR 后回填 `pr_url` 与状态。
	6) 同步更新 `sdd-index.md` 矩阵。
- 明确命名规范：
	- 目录名：英文小写 + 短横线。
...
## 完成标准
- [ ] SDD 总规范文档完成并可作为唯一入口。
- [ ] 模板清单与选型规则明确，字段定义可直接复用。
- [ ] 日期目录、命名规范、状态字段、工作流、DoD 均有明确条目。
- [ ] `sdd-index` 同步机制被定义，可映射 issue/pr 生命周期。

## AI 上下文

- 该任务核心是“规范化与可追踪”，不是新增业务功能。
- 输出内容要能被 AI 与人工共同执行，避免模糊约定。
...
- 推荐在仓库根目录新增 SDD 规范入口，并在 `sdd-index.md` 维护状态矩阵。
- 推荐新增文档检查脚本（后续任务）验证命名、字段、目录与链接完整性。
