...
如果你是进入仓库执行任务的 code agent，先读 [AGENTS.md](AGENTS.md)。它汇总了默认执行流程、角色边界、门禁要求和关键文件索引，可以把冷启动成本压到最低。
...
1. 人工驱动模式（监督式）
  - 先用 `.github/manual/scripts/prepare-single-issue.py` 准备单个 Issue 工作区和 prompt
  - 人工在 Chat 中逐步驱动 Agent 执行，并在关键步骤做审核
  - 适合高风险需求、需求尚在澄清阶段、或需要强监督的任务

2. 自动化模式（批处理）
  - 使用 Celery dispatcher + worker 统一调度与执行
  - 适合高并发和依赖排产场景，支持按 `Depends on: #123` 分层调度

Celery 模式建议命令：

```bash
# 终端 1：启动 worker
./.github/automation/scripts/run-celery-worker.sh

# 终端 2：启动 dispatcher（循环模式）
./.github/automation/scripts/run-celery-dispatch.sh

# 单次调度
ONCE=true ./.github/automation/scripts/run-celery-dispatch.sh
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .github/manual/scripts/prepare-single-issue.ps1 -IssueNumber 6

# 从本地 issue/backlog 文件准备单 issue 工作区
powershell -NoProfile -ExecutionPolicy Bypass -File .github/manual/scripts/prepare-single-issue.ps1 -IssueFile .github/issues-backlog/task/20260602/validate-automation-runner-e2e.md
```
